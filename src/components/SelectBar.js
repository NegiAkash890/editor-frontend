...
const SelectBar = ({ changeLanguage }) => {
  ...
  const handleClick = (e, value) => {
    switch (value) {
      case 'cpp':
        e.target.setAttribute('src', `${process.env.PUBLIC_URL}/assets/c-active.png`);
        setLang('cpp');
        setLanguage('cpp');
        break;
      case 'java':
        e.target.setAttribute('src', `${process.env.PUBLIC_URL}/assets/java-active.png`);
        setLang('java');
        setLanguage('java');
        break;
      case 'py':
        e.target.setAttribute('src', `${process.env.PUBLIC_URL}/assets/py-active.png`);
        setLang('py');
        setLanguage('python');
        break;
      default:
